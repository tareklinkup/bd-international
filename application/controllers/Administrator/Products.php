    ...
    public function getProducts(){
        ...
        $products = $this->db->query("
            select
                ...
                br.brand_name,
                u.Unit_Name,
                ( select ifnull (sum((select (ci.purchase_quantity + ci.sales_return_quantity + ci.transfer_to_quantity) - (ci.sales_quantity + ci.purchase_return_quantity + ci.damage_quantity + ci.transfer_from_quantity))), 0)                  
                from tbl_currentinventory ci
                where ci.product_id = p.Product_SlNo
                and ci.branch_id = '$this->brunch'
                ) as current_quantity
            from tbl_product p
