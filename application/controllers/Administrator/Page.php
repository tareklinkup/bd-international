...
class Page extends CI_Controller {
    ...
    public function addBranch()  {
        ...
        try{
            ...
            if(!empty($_FILES['headerImage'])) {
                $temp = explode(".", $_FILES["headerImage"]["name"]);
                $newfilename = round(microtime(true)) . '.' . end($temp);
                $file_location = './uploads/invoice/header/' . $newfilename;
                move_uploaded_file($_FILES['headerImage']['tmp_name'], $file_location);
                $file_location = '/uploads/invoice/header/' . $newfilename;
                $this->db->query("update  tbl_brunch set header_logo = ? where brunch_id = ?", [$file_location, $branchId]);
            }
    ...
    public function updateBranch()  {
        ...
        try{
            ...
            $branchInfo = $this->db->query("select * from tbl_brunch where brunch_id = ?", [$branch->branchId])->row();
            $branchId = $branch->branchId;

            if(!empty($_FILES['headerImage'])) {
                if(file_exists($branchInfo->header_logo)){
                    unlink($branchInfo->header_logo);
                }
                $temp = explode(".", $_FILES["headerImage"]["name"]);
                $newfilename = round(microtime(true)) . '.' . end($temp);
                $file_location = './uploads/invoice/header/' . $newfilename;
